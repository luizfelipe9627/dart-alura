...
class AccountService {
  ...
  Future<List<Account>> getAll() async {
    ...
    Response response = await get(
      ...
      headers: {
        ...
        // O cabeçalho Authorization é utilizado para autenticar a requisição utilizando o token de acesso pessoal do GitHub
      },
    );
  ...
  save(List<Account> listAccounts, {String accountName = ""}) async {
    ...
    Response response = await post(
      Uri.parse(url),
      // Faz a conversão da URL em um objeto Uri para ser utilizado na requisição
      ...
      body: json.encode({
        ...
        "files": {
          "accounts.json": {
            "content": content,
            // O conteúdo é o corpo da requisição que contém o JSON
          },
        },
      }),
      // O cabeçalho da requisição é um mapa que contém informações adicionais sobre a requisição
